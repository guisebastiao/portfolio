...
export const DatabaseStackCard = () => {
  const { t } = useTranslation("about");

  const { stacks } = DEV_STACKS.filter((stack) => stack.type === "database")[0];

  return (
    <motion.div className={CARD_CLASSNAME} {...motionVariantHelper(fadeSlide({ y: 50 }))}>
      <div className="flex items-center gap-1.5 mb-3">
        <Database className="size-4.5 text-foreground/85" />
        <h1 className="text-[15px] font-medium tracking-wide text-foreground/85">
          {t("card-database.title")}
        </h1>
      </div>
